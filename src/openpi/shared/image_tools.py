...
def resize_with_pad(
    ...
    if images.ndim < 3:
        raise ValueError(f"resize_with_pad expects at least 3D (h, w, c) input, got shape {orig_shape}")
    if images.ndim == 3:
        flat = images[None]
    else:
        flat = images.reshape(-1, *images.shape[-3:])
    cur_height, cur_width = flat.shape[1:3]
    ratio = max(cur_width / width, cur_height / height)
    resized_height = int(cur_height / ratio)
    resized_width = int(cur_width / ratio)
    resized_images = jax.image.resize(
        flat, (flat.shape[0], resized_height, resized_width, flat.shape[3]), method=method
    )
    if flat.dtype == jnp.uint8:
        ...
        raise ValueError(f"Unsupported image dtype: {flat.dtype}")
    ...
    padded_images = jnp.pad(
        ...
        constant_values=0 if flat.dtype == jnp.uint8 else -1.0,
    )

    if images.ndim == 3:
        return padded_images[0]
    return padded_images.reshape(*orig_shape[:-3], height, width, orig_shape[-1])
